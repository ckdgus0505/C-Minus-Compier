...
char *s_kind[5] = {"global", "local", "argument", "function", "functioni"};
char *s_type[4] = {"int", "int array", "int *", "void"};
...
void *free_table(struct table *tablep)
{
  free(tablep);
}
...
void print_table(struct table *tablep)
{
  ...
  for (i = 0; i < HASH_SIZE; i++)
    ...
    {
      fprintf(stdout, "%-10s %-10s %-10s %-10s %10d %10d\n", tablep->name, symbolp->name, s_kind[symbolp->kind], s_type[symbolp->type], symbolp->size, symbolp->offset);
    }
}
